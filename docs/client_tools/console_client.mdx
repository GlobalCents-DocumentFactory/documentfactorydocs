...
of the DocumentFactory installation process. This directory also contains additional configuration files required for processing,
...
- `install` -	Installs the DocumentFactory client as a service
- `uninstall` -	Uninstalls the DocumentFactory client service
- `run` -	Runs the client as a background service
- `process` -	Processes a single document or a batch of documents
- `processfolders` -	Processes each individual folder in a folder hierarchy
- `completion` -	Generates an autocompletion script for supported shells
...
- **[**NATS**](/docs/next/getting_started/terminology%20_and_concepts#nats) messaging settings**
  ...
  This key is not secret and can safely be shared with the server.
  ...
  This key is highly sensitive and must be kept secret.
...
- **MinIO integration**
    ...
    - `MINIO_ACCESSKEY`, `MINIO_SECRETKEY` – credentials for accessing the MinIO storage.
