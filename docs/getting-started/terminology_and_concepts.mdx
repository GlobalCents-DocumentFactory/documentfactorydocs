...
A task is the smallest unit of execution within a workflow. It represents a discrete operation performed on a document
or a group of documents. During execution, the workflow is decomposed into multiple tasks, one for each combination
of defined operations and target documents, enabling parallelism and precise control over processing.
