...
Create one or more pipelines (see [**how to build a pipeline**](../category/pipeline)) that define the sequence
of processing steps to apply to incoming documents - see [**pipeline definition**](./terminology_and_concepts#pipeline)
for more details on what a pipeline is.  Pipelines are managed via the Web Client or defined using configuration files.
...
MinIO (used as an S3-compatible object store). Metadata, logs, and status information are retained for monitoring and auditing purposes.
