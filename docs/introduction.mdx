...
on **Windows (Azure)**, **Linux**, or **Kubernetes** — making it easy to standardize and automate document workflows.
...
of input types and output formats. With features such as OCR, compression, merging, and watermarking,
files can be transformed and prepared for archiving, sharing, or compliance with minimal manual intervention.
...
**GCI DocumentFactory** provides the reliability and flexibility needed to manage documents efficiently and effectively.

The sections that follow provide everything needed to get started—from installation and configuration to API references
