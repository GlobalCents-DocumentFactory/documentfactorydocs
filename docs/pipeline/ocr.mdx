...
OCR allows such documents to be transformed into editable data that can be archived, searched, or further automated.
It can be enabled in the document processing pipeline by setting the `actionocr` property to true within either
the `processors` or `postprocess` arrays.
