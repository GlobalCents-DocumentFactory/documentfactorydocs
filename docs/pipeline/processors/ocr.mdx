...
By setting this property to true within the `processors` or `postprocess` arrays, you enable the OCR functionality,
which allows the system to recognize and extract text from images or scanned documents.

This property is particularly useful for converting non-editable text, such as that found in scanned documents or images,
into editable and searchable text. Ensuring the `actionocr` property is set correctly enhances the document processing
workflow by enabling text recognition capabilities.
...
- <Link to="#ocrdevice">`saveformat`</Link> - specifies the OCR device to be used for processing.
- <Link to="#downscalefactor">`saveformat`</Link> - determines the factor by which the original image is downscaled
