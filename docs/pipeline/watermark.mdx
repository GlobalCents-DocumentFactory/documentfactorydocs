...
The `actionwatermark` setting activates the watermarking feature within the document processing pipeline.
By setting this property to `true` within the `processors` or `postprocessors` arrays, the system applies a watermark
to documents according to the specified parameters.
...
Defines the font used for text-based watermarks. Its value must correspond to one of the supported font families
...
Specifies the content displayed as the watermark on the document. It defines the exact text that will
...
Determines whether the watermark should be placed under the document layout.  If the `underlayout` property is set to `true`,
...
Determines the horizontal margin around the watermark, defining the amount of space between the edges of the watermark
and the left and right borders of the document. This setting controls the horizontal positioning of the watermark within
the document, with the margin measured in points.
...
Determines the vertical margin around the watermark, defining the space between the edges of the watermark and the top
and bottom borders of the document. This controls the vertical positioning of the watermark within the document,
with the margin measured in points.
...
Defines the degree of rounding for the watermark border corners, measured in points.
...
Determines whether the watermark background is transparent. When enabled, the area behind the watermark remains visible.
Otherwise, it is covered by a solid background.
...
Specifies the transparency level of the watermark. It controls how visible or faint the watermark appears on the document.
The value for this property should be a number between 0 and 1, where 0 represents complete transparency (fully invisible)
and 1 represents full opacity (fully visible).
...
Determines the type of barcode to be generated and applied within the document. Supports a wide variety of standards and
options such as:
...
Specifies whether the human-readable text representation of the barcode appears below it.
...
Determines the scaling factor for the barcode, adjusting its size inside the document.
